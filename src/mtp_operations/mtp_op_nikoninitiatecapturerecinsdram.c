...
uint32_t mtp_op_NikonInitiateCaptureRecInSDRAM(mtp_ctx * ctx,MTP_PACKET_HEADER * mtp_packet_hdr, int * size,uint32_t * ret_params, int * ret_params_size)
{
	...
	if (sizeof(MTP_PACKET_HEADER) == *size) {
		...
		PRINT_DEBUG("MTP_OPERATION_NIKON_INITIATE_CAPTURE_REC_IN_SDRAM : storage id 0x%x", storageid);
		storage_path = mtp_get_storage_root(ctx, storageid);
	}

        if (storage_path == NULL) 
        {
		PRINT_DEBUG("MTP_OPERATION_NIKON_INITIATE_CAPTURE_REC_IN_SDRAM : Invalid storage id 0x%x", storageid);
                return MTP_RESPONSE_INVALID_STORAGE_ID;
        }

	PRINT_DEBUG("MTP_OPERATION_NIKON_INITIATE_CAPTURE_REC_IN_SDRAM : storage path %s", storage_path);

	// Take photo
	ctx->NikonInitiateCaptureTxId = mtp_packet_hdr->tx_id;
	pthread_create(&thread_id, NULL, capture, storage_path);

	PRINT_DEBUG("MTP_OPERATION_NIKON_INITIATE_CAPTURE_REC_IN_SDRAM : Success!");
